...
def obtener_datos_gui():
    ...
    def submit_info():
        ...
        info_adicional = entry_otro.get()  # Obtener texto de la casilla opcional
        if nombre_voluntario and button_number is not None:
            root.quit()
        else:
            label_error.config(text="Por favor, complete todos los campos.")

    # Crear una ventana para seleccionar el número del botón y el nombre del voluntario
    root = tk.Tk()
    ...
    label_nombre = tk.Label(root, text="Ingrese el nombre del voluntario:")
    ...
    for i in range(1, 8):
        button = tk.Button(button_frame, text=f"Botón {i}", command=lambda i=i: on_button_click(i))
        button.pack(side=tk.LEFT, padx=5)

    # Etiqueta y campo de entrada opcional
    label_otro = tk.Label(root, text="Información adicional (opcional):")
    label_otro.pack(padx=10, pady=5)

    entry_otro = tk.Entry(root)
    entry_otro.pack(padx=10, pady=5)

    # Botón para enviar la información
    submit_button = tk.Button(root, text="Enviar", command=submit_info)
