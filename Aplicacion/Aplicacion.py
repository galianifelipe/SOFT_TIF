...
# Usar siempre la carpeta del ejecutable (cuando esté empacado con PyInstaller será la carpeta del .exe;
# si ejecutas el .py directamente, esto también usará la carpeta del ejecutable de Python — pedido explícito).
BASE_DIR = Path(sys.executable).resolve().parent

# Carpeta Medidas está al mismo nivel que Aplicacion (SOFT_TIF/Medidas)
MED_DIR = BASE_DIR.parent / "Medidas"
print(BASE_DIR)
...
def cargar_k():
    archivo = str(BASE_DIR / "calibracion.txt")
...
def obtener_datos_gui():
    ...
    def abrir_calibracion():
        global k_resorte
        ruta_imagen = str(BASE_DIR / "im.png")
    ...
    ruta_icono = str(BASE_DIR / "icono.ico")
    try:
    ...
    ruta_fondo = str(BASE_DIR / "Fondo.png")
    try:
    ...
    try:
        ...
        pass
    return nombre_voluntario, button_number, info_adicional, globals().get("k_resorte", None)
...
# Bucle principal: volver a la ventana inicial después de cerrar la medición.
while True:
    nombre_voluntario, button_number, otro, k_from_gui = obtener_datos_gui()
    # recargar k por si se calibró
    k_resorte = cargar_k()

    # Si el usuario cerró la ventana inicial sin completar, terminar el programa
    if nombre_voluntario is None:
    ...
    # determinar lugar correctamente
    if button_number is not None and button_number != 0:
         lugar=optn[button_number]
    else:
         lugar = otro

    carpeta_destino = str(MED_DIR)
    ...
    # Esperar hasta que el usuario cierre la ventana de medición o presione ESC.
    try:
        while True:
            if cv2.getWindowProperty('DOLORIMETRO', cv2.WND_PROP_VISIBLE) < 1:
                break
            if (cv2.waitKey(30) & 0xFF) == 27:
                break
    finally:
        try:
            if cv2.getWindowProperty('DOLORIMETRO', cv2.WND_PROP_VISIBLE) >= 0:
                cv2.destroyWindow('DOLORIMETRO')
        except cv2.error:
            pass

    # procesar las medidas recogidas en 'a'
    for i in a:
    ...
    # después de completar la medición, el bucle vuelve a abrir la ventana inicial